  ...
  if (!timeLeft && timeLeft !== 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full">
        <div className="text-lg text-slate-500 dark:text-slate-400">{noTaskMessage}</div>
      </div>
    );
  }

  const percentage = timeLeft && totalTime ? Math.max(0, Math.min(100, (timeLeft / totalTime) * 100)) : 0;
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full">
      ...
      <div className="w-32 h-48 relative mb-2">
        ...
        <div className="absolute -top-3 -left-4 w-6 h-6 text-yellow-400 animate-pulse">✦</div>
        <div className="absolute -top-5 left-8 w-6 h-6 text-yellow-400 animate-pulse" style={{ animationDelay: '0.5s' }}>⭐</div>
        <div className="absolute -top-3 -right-4 w-6 h-6 text-yellow-400 animate-pulse" style={{ animationDelay: '1.2s' }}>✦</div>
        
        {/* Background frame */}
        <div className="absolute inset-0 border-4 border-slate-300 dark:border-slate-600 rounded-3xl backdrop-blur-sm bg-white/20 dark:bg-slate-800/20 overflow-hidden">
            ...
            {!isPaused && Array.from({ length: 8 }).map((_, i) => (
              <div 
                ...
                style={{
                  ...
                  animation: `float ${animationDuration + Math.random()}s ease-in-out infinite alternate`,
            ...
            <div 
              ...
              style={{
                background: isPaused ? 
                  ...
                  `linear-gradient(135deg, ${timerColor} 25%, ${adjustColor(timerColor, 30)} 50%, ${timerColor} 75%)`,
                backgroundSize: isPaused ? '30px 30px' : '20px 20px',
                animation: isPaused ? 'none' : 'sandAnimation 2s linear infinite',
              }}
            />
            ...
            {!isPaused && Array.from({ length: 5 }).map((_, i) => (
              <div 
                ...
                style={{
                  ...
                  backgroundColor: adjustColor(timerColor, Math.random() * 20),
                  left: `${Math.random() * 80 + 10}%`,
                  animation: `fall ${animationDuration - 0.5}s ease-in infinite`,
