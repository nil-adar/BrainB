...
useEffect(() => {
  ...
  const getCategoryStyles = () => {
    const baseStyles = "border rounded-xl shadow-sm hover:shadow-md transition-all duration-300 flex items-center gap-3 p-3 mb-3 cursor-pointer";
    ...
    const selectedStyles = isSelected ? "ring-2 ring-offset-2 ring-blue-500 dark:ring-blue-400 shadow-md" : "";
    const pausedStyles = isSelected && isPaused ? "ring-amber-500 dark:ring-amber-400" : "";
    return cn(baseStyles, categoryStyles[category], selectedStyles, pausedStyles);
  };
  ...
  const handleAttemptComplete = (e: React.MouseEvent) => {
    ...
    if (!hasUploadedImage && !completed) {
      ...
      const uploadButton = e.currentTarget.previousElementSibling?.previousElementSibling;
      if (uploadButton instanceof HTMLElement) {
        uploadButton.classList.add("animate-pulse", "ring", "ring-red-500");
        setTimeout(() => {
          uploadButton.classList.remove("animate-pulse", "ring", "ring-red-500");
        }, 2000);
      }
      return;
    }
  ...
  return (
    <div className="flex flex-col">
     ...
     <div className={`w-10 h-10 rounded-full ${getTailwindBgColor(color)} flex items-center justify-center`}>
  {}
</div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            className={cn(
              ...
              completed ? "bg-green-500 border-green-500 text-white" : "bg-white dark:bg-slate-800"
            )}
